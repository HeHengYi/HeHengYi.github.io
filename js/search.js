    ...
    complete: function (xmlResponse) {
      ...
      searchAll = function (val) {
        ...
        datas.forEach(function (data) {
          ...
          // show search results
          if (isMatch) {
			      data_url = decodeURIComponent(data_url);
            var urls = data_url.split("/");
            if (!data_url.startsWith("/")) {
              data_url = "/" + data_url;
            }
			      // 分系统展示
			      // if ($(".book a:first-child").html() != urls[1]) return true;
            var post_date = urls[1]+"/"+urls[2]+"/"+urls[3];
            str += "<li><a href='" + data_url + "'><span class='iconfont icon-arrow-right post-title' title='"+data_title+"'>" + data_title + "</span></a>";
